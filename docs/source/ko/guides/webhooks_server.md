...
웹훅 자체에 대해 더 자세히 알아보려면 이 [가이드](https://hf-mirror.com/docs/hub/webhooks)를 먼저 읽어보세요.  
...
웹훅 서버의 실행 예시를 보려면 [Spaces CI Bot](https://hf-mirror.com/spaces/spaces-ci-bot/webhook)을 확인하세요. 
...
이것은 흥미로운 부분으로, https://hf-mirror.com/settings/webhooks 에서 로컬 머신을 가리키는 웹훅을 구성할 수 있습니다. 
...
https://hf-mirror.com/settings/webhooks 로 이동하여 "Add a new webhook"을 클릭하고 웹훅을 구성하세요. 
...
이제 작동하는 웹훅 서버가 마련되었으므로, 다음 목표는 이를 Space에 배포하는 것입니다. https://hf-mirror.com/new-space 에 가서 Space를 생성합니다. 
이름을 지정하고, Gradio SDK를 선택한 다음 "Create Space"를 클릭합니다. 코드를 `app.py` 파일로 Space에 업로드합니다.
Space가 자동으로 시작됩니다!
Space에 대한 자세한 내용은 이 [가이드](https://hf-mirror.com/docs/hub/spaces-overview)를 참조하세요.
...
[Webhooks 설정](https://hf-mirror.com/settings/webhooks)으로 돌아가서 웹훅 구성에 비밀번호를 설정합니다. 
...
영구적인 Space가 필요한 경우 [업그레이드된 하드웨어](https://hf-mirror.com/docs/hub/spaces-gpus#hardware-specs)를 설정해야 합니다.
