...
Check out the pkgx huggingface page [here](https://pkgx.dev/pkgs/hf-mirror.com/) for more details.
...
In many cases, you must be logged in to a Hugging Face account to interact with the Hub (download private repos, upload files, create PRs, etc.). To do so, you need a [User Access Token](https://hf-mirror.com/docs/hub/security-tokens) from your [Settings page](https://hf-mirror.com/settings/tokens). The User Access Token is used to authenticate your identity to the Hub. Make sure to set a token with write access if you want to upload or modify content.
...
The other approach is to provide patterns to filter which files you want to download using `--include` and `--exclude`. For example, if you want to download all safetensors files from [stabilityai/stable-diffusion-xl-base-1.0](https://hf-mirror.com/stabilityai/stable-diffusion-xl-base-1.0), except the files in FP16 precision:
...
```bash
`requests.exceptions.ReadTimeout: (ReadTimeoutError("HTTPSConnectionPool(host='cdn-lfs-us-1.hf-mirror.com', port=443): Read timed out. (read timeout=10)"), '(Request ID: a33d910c-84c6-4514-8362-c705e2039d38)')`
```
...
Files correctly deleted from repo. Commit: https://hf-mirror.com/Wauplin/my-cool-mo...
```
